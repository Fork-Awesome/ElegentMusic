...
var SongList = React.createClass({displayName: "SongList",
    ...
    render: function() {
        var songNodes = this.props.data.map(function(song) {
            console.log(song.from);
            return (
                React.createElement(SongInfo, {attrFrom: song.from, songId: song.id, songName: song.name, songSinger: song.singer})
...
var SongInfo = React.createClass({displayName: "SongInfo",
    render: function() {
        return (
            React.createElement("div", {onclick: "playMusic", attrFrom: this.props.attrFrom, id: this.props.songId, className: "SongInfo"}, 
